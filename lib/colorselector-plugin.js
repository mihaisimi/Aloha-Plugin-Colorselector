            ...
            */
            onColorSelect: function(colorCode, colorArg) {
                ...
                foundMarkup = rangeObject.findMarkup(function() {
                    var retval;
                    try {
                      retval = this.nodeName.toLowerCase() == markup.get(0).nodeName.toLowerCase() && this.style[0].toLowerCase == markup.get(0).style.toLowerCase();
                    }
                    catch(err) {
                      retval = false;
                    }
                ...
                // always add the markup since clearing something doesn't just delete the prior markup
