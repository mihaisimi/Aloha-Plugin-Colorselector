...
function (Aloha, jQuery, Plugin, Ui, Button, i18n, i18nCore){
  ...
  return Plugin.create( 'colorselector', {
    ...
    init: function() {
      ...
      if(!Aloha.settings.plugins.colorselector.colors) {
        Aloha.settings.plugins.colorselector.colors =
      ...
      var that = this,
          buttons = [],
          names = ['colorselector', 'fontcolorselector' ],
          tooltips = [i18n.t('button.colorselector.tooltip'), i18n.t('button.fontcolorselector.tooltip') ],
          actions = ['highlight', 'color' ];

      function onColorSelect(colorCode, colorArg) {
        var self = this;
        var tagToUse = "span";
        ...
        _initHideOnDocumentClick: function() {
            ...
            jQuery('body').click(function(e) {
                var overlayVisibleAndNotTarget
                    ...
                    && !(jQuery(e.target).hasClass('aloha-button-colorselector') || jQuery(e.target).children().hasClass('aloha-button-colorselector'))
                    && !(jQuery(e.target).hasClass('aloha-button-fontcolorselector') || jQuery(e.target).children().hasClass('aloha-button-fontcolorselector'))
      ...
      var colorselector = new Colorselector(Aloha.settings.plugins.colorselector.colors, onColorSelect);
      for (var i=0; i < names.length; i++) {
        ...
        Ui.adopt(value, Button, {
          ...
          click: function () {
              ...
              colorselector.show(this.buttonElement[0], actions[index]);
          }
        });
      }
    }
